...
void foo() {
  ...
  for (ll i = 0; i < n; ++i) {
    for (ll j = n - 1; j >= 0; --j) {
      ...
      if (s.find(arr[i] % arr[j]) == s.end() and
          sp.find({arr[i], arr[j]}) == sp.end()) {
        --cnt;
        sp.insert({arr[i], arr[j]});
      }
      if (cnt == 0) {
        break;
      }
      if (s.find(arr[j] % arr[i]) == s.end() and
          sp.find({arr[j], arr[i]}) == sp.end()) {
        --cnt;
        sp.insert({arr[j], arr[i]});
      }
    }
  }
